...
{
    //$fgmembersite->RedirectToURL("device_settings_admin.php");
    //exit;
}
if(isset($_POST['addusergroup']))
{
	$fgmembersite->RedirectToformURL($fgmembersite->UserGroupAdd($_POST['addusergroup']),$_SERVER['PHP_SELF'],'formrespond.php');
	exit;
}
if(isset($_POST['delusergroup']))
{
	$fgmembersite->RedirectToformURL($fgmembersite->UserGroupDelete($_POST['delusergroup']),$_SERVER['PHP_SELF'],'formrespond.php');
	exit;
}
...
{
	$fgmembersite->RedirectToformURL($fgmembersite->UserGroupAssigne($_POST),$_SERVER['PHP_SELF'],'formrespond.php');
	exit;
}
if(isset($_POST['user']))
{
...
if(isset($_POST['objectsubmit']))
{
	if($_POST['reson'] == "new")
	{
		$fgmembersite->RedirectToformURL("Please contact system administrator!",$_SERVER['PHP_SELF'],'formrespond.php');
		exit;
	}
...
function delconfirmgroup(id){
  ...
  document.getElementById('usergroup'+id).submit();
  }
}
function addconfirmusergroup(){
  if (isCharactorValide(document.getElementById('addusergroup').value)) {
  document.getElementById('usergroupadd').submit();
  }else{
	alert("Group name not valide. Special charactors and Blank names are not allowed!");
  }
}
function usergroupaddsubmit(value){
	document.getElementById('usergroupaddform'+value).submit();
}
...
function validateForm() {
	...
	{
		alert("You can't add new devices. please contact system administrator!");
		return false;
	}
	/*
		...
		<div class="col-lg-12">
					<div class="hpanel">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="data_Device">
                                ...
                                <tbody>
								<?php
									echo $fgmembersite->ObjectManagementList("normal");
								?>
                                </tbody>
                            </table>
					</div>
	...
	<div class="row">
		<div class="col-lg-10">
					<div class="hpanel">
						...
						<div class="form-group" style="display: none;"><label class="col-sm-2 control-label">Object Number</label>
						...
						<div class="form-group" style="display: none;"><label class="col-sm-2 control-label">Model</label>
							<div class="col-sm-10">
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="data_Device1">
                                <thead>
                                    <tr>
                                        <th>Vehicle</th>
										<th>Current Group</th>
                                        <th data-hide="phone,tablet">Change group</th>
                                    </tr>
                                </thead>
                                <tbody>
								<?php
									echo $fgmembersite->UserassignedObjects();
								?>
                                </tbody>
                            </table>
					</div>
...
<div class="content">
	<div class="row">
		<div class="col-lg-10">
					<div class="hpanel">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="data_Device2">
                                <thead>
                                    <tr>
                                        <th> User Groups</th>
                                        <th data-hide="phone,tablet">Activity</th>
                                ...
                                <tbody>
								<?php
									echo $fgmembersite->UserGroupeTable();
								?>
								<td>
								<form id='usergroupadd' action='<?php echo $_SERVER['PHP_SELF']; ?>' method='post'><input type='text' id='addusergroup' name='addusergroup' value=''/></form>
								</td>
								<td>
								<a class='btn btn-primary btn-sm' onclick="addconfirmusergroup('admingroupadd')" ><i class='glyphicon glyphicon-edit icon-white'></i>Add Group</a>
								</td>
                                </tbody>
                            </table>
					</div>
